...
func SeverDiscover() (*grpc.ClientConn, error) {
	// 连接consul
	cfg := api.DefaultConfig()
	cfg.Address = "101.37.25.231:8500"//注册中心
	...
	serviceMap, err := cc.Agent().ServicesWithFilter("Service==`local_test`")
	if err != nil {
...
func main() {
	flag.Parse()
	// 连接到server端
	conn, err := SeverDiscover()
	if err != nil {
		fmt.Printf("Sever discover failed, err:%v\n", err)
	}
	defer conn.Close()
